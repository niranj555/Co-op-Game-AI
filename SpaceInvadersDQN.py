...
EXPLORE             = 1000000 # timesteps to train
FINAL_EPSILON       = 0.1 # final value of exploration rate
INITIAL_EPSILON     = 1  # starting value of exploration rate
REPLAY_MEMORY       = 100000 # number of experiences stored
...
def trainNetwork(s, readout, sess):
	...
	while True:
		...
		if terminal_t:
			...
			f = open("training_data_single.txt","a+")
			end_time = time.time()
			play_time = end_time - start_time

			f.write(str(play_time) + "," + str(random_action_count) + "," + str(epsilon) +"," + str(t) + "\n")
			start_time = end_time
			random_action_count = 0
			f.close()
			p.reset_game()
		...
		if MODE == 'train':
			...
			if epsilon > FINAL_EPSILON and t > OBSERVE:
				...
				#print("Epsilon reduced to:",epsilon)
			# else:
			# 	epsilon = FINAL_EPSILON
			# 	print("Reached final epsilon")
